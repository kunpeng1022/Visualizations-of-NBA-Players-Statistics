...
function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    vector_year <- character()
    for (i in (input$start_year):(input$end_year)){
      vector_year <- c(vector_year, as.character(i))
    }
    if (input$start_year < input$end_year){
      dataset <- data_select %>% filter(Year %in% vector_year)
    }
